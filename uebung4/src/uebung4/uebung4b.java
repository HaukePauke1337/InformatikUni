...
public class uebung4b {
	...
	public static double Laufzeit() {
		...
		double Laufzeit;
		SimpleIO.println("Bitte die Laufzeit der Untersuchung in Minuten angeben");
		Laufzeit = SimpleIO.readDouble();
		Laufzeit = Laufzeit * 60 * 10;
	...
	// Mit dieser Funktion wird überprüft, ob die Temperatur sinkt oder steigt, um
	// anschließend
	// einen String auszugeben. Da die Temperatur sowohl bei Wind als auch ohne Wind
	// steigt,
	// ist die Funktion überflüssig. Ich will sie aber behalten weil sie ja jetzt
	// schon drin ist :D
	...
	public static void main(String[] args) {
		...
		if (WoderWS) {
			...
			SimpleIO.println("\nSomit ist die Temperatur um " + Math.abs(Tempwind - Stahltempanfang) + " °C "
					+ Vorzeichen(Tempwind));
			...
			SimpleIO.println("\nSomit ist die Temperatur um " + Math.abs(Tempstill - Stahltempanfang) + " °C "
					+ Vorzeichen(Tempstill));
		}
